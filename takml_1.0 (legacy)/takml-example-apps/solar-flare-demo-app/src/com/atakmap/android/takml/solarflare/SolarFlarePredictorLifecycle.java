...
package com.atakmap.android.takml.solarflare;
...
import com.atakmap.android.takml.SolarFlarePredictorMapComponent;
...
public class SolarFlarePredictorLifecycle implements Lifecycle {
    ...
    private final static String TAG = SolarFlarePredictorLifecycle.class
            .getSimpleName();

    public SolarFlarePredictorLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        SolarFlarePredictorLifecycle.this.overlays
                .add(new SolarFlarePredictorMapComponent());

        activity = arg0;

        // create components
        Iterator<MapComponent> iter = SolarFlarePredictorLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(SolarFlarePredictorLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        SolarFlarePredictorLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

...
public class GenericImageRecognitionLifecycle implements Lifecycle {
    ...
    private final static String TAG = GenericImageRecognitionLifecycle.class
            .getSimpleName();

    public GenericImageRecognitionLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        GenericImageRecognitionLifecycle.this.overlays
                .add(new GenericImageRecognitionMapComponent());

        activity = arg0;

        // create components
        Iterator<MapComponent> iter = GenericImageRecognitionLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(GenericImageRecognitionLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        GenericImageRecognitionLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

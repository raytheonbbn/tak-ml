...
package com.atakmap.android.takml.sensor.plugin;
...
import com.atakmap.android.takml.sensor.SensorExamplesMapComponent;
...
public class SensorExamplesLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = "SensorExamplesLifecycle";

    public SensorExamplesLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        SensorExamplesLifecycle.this.overlays
                .add(new SensorExamplesMapComponent());

        // create components
        Iterator<MapComponent> iter = SensorExamplesLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(SensorExamplesLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        SensorExamplesLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

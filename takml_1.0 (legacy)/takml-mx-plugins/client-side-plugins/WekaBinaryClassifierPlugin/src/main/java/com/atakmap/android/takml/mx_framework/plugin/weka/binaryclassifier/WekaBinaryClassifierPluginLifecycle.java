...
import com.atakmap.android.takml.mx_framework.plugin.weka.WekaBinaryClassifierPluginMapComponent;
...
public class WekaBinaryClassifierPluginLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = WekaBinaryClassifierPluginLifecycle.class.getSimpleName();

    public WekaBinaryClassifierPluginLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        WekaBinaryClassifierPluginLifecycle.this.overlays
                .add(new WekaBinaryClassifierPluginMapComponent());

        // create components
        Iterator<MapComponent> iter = WekaBinaryClassifierPluginLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(WekaBinaryClassifierPluginLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        WekaBinaryClassifierPluginLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

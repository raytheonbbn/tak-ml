...
import com.atakmap.android.takml.mx_framework.WekaPluginMapComponent;
...
public class WekaPluginLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = WekaPluginLifecycle.class.getSimpleName();

    public WekaPluginLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        WekaPluginLifecycle.this.overlays
                .add(new WekaPluginMapComponent());

        // create components
        Iterator<MapComponent> iter = WekaPluginLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(WekaPluginLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        WekaPluginLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

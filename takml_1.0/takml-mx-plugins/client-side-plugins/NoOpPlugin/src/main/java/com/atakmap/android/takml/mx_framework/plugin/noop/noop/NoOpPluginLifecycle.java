...
import com.atakmap.android.takml.mx_framework.plugin.noop.NoOpPluginMapComponent;
...
public class NoOpPluginLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = NoOpPluginLifecycle.class.getSimpleName();

    public NoOpPluginLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        NoOpPluginLifecycle.this.overlays
                .add(new NoOpPluginMapComponent());

        // create components
        Iterator<MapComponent> iter = NoOpPluginLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(NoOpPluginLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        NoOpPluginLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

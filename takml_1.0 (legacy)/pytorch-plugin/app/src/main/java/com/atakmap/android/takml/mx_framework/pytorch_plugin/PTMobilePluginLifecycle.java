...
import com.atakmap.android.takml.mx_framework.PTMobilePluginMapComponent;
...
public class PTMobilePluginLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = PTMobilePluginLifecycle.class.getSimpleName();

    public PTMobilePluginLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        PTMobilePluginLifecycle.this.overlays
                .add(new PTMobilePluginMapComponent());

        // create components
        Iterator<MapComponent> iter = PTMobilePluginLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(PTMobilePluginLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        PTMobilePluginLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

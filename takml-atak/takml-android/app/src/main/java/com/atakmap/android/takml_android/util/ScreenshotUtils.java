...
public class ScreenshotUtils {
    ...
    /**
     * Takes a screenshot of the ATAK Map
     *
     ...
     */
    public static void takeScreenshot(ImageryCaptureTask.Callback callback) {
        ...
        int w = MapView.getMapView().getWidth(), h = MapView.getMapView().getHeight();
        GeoPoint[] quad = new GeoPoint[] {
                MapView.getMapView().inverse(new PointF(0, 0)).get(), // Top-left corner
                MapView.getMapView().inverse(new PointF(w, 0)).get(), // Top-right corner
                MapView.getMapView().inverse(new PointF(w, h)).get(), // Bottom-right corner
                MapView.getMapView().inverse(new PointF(0, h)).get() // Bottom-left corner
        };
        ...
        new ImageryCaptureTask(tc, cp, new ImageryCaptureTask.Callback() {
            @Override
            public void onFinishCapture(TileCapture tileCapture, ImageryCaptureParams imageryCaptureParams, File imageryFile) {
                ...
                MapItemCapturePP postDraw = new MapItemCapturePP(MapView.getMapView(), tc, cp);
                ...
                callback.onFinishCapture(tileCapture, imageryCaptureParams, jpegFile);
            }
        }).execute();
    }
}

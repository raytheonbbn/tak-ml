...
package com.atakmap.android.takml.occupancy;
...
import com.atakmap.android.takml.OccupancyDetectorMapComponent;
...
public class OccupancyDetectorLifecycle implements Lifecycle {
    ...
    private final static String TAG = OccupancyDetectorLifecycle.class
            .getSimpleName();

    public OccupancyDetectorLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        OccupancyDetectorLifecycle.this.overlays
                .add(new OccupancyDetectorMapComponent());

        activity = arg0;

        // create components
        Iterator<MapComponent> iter = OccupancyDetectorLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(OccupancyDetectorLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        OccupancyDetectorLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,

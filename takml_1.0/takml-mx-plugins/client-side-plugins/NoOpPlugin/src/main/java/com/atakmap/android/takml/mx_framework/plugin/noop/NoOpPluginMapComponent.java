...
package com.atakmap.android.takml.mx_framework.plugin.noop;
...
import com.atakmap.android.takml.mx_framework.plugin.noop.noop.NoOpPlugin;
import com.atakmap.coremap.log.Log;
import com.atakmap.android.takml.mx_framework.plugin.noop.noop.R;
...
public class NoOpPluginMapComponent extends DropDownMapComponent {

    private static final String TAG = NoOpPluginMapComponent.class.getSimpleName();
    ...
    private void register() {
        if (!this.mxf.register(NoOpPlugin.class)) {
            Log.e(TAG, "Could not register plugin with MX framework, retrying...");
            this.reregisterTimer.schedule(new ReregisterTask(), REREGISTER_INTERVAL_MS);
        }
    }

    @Override
    protected void onDestroyImpl(Context context, MapView view) {
        if (!this.mxf.deregister(NoOpPlugin.class))
